...
presidents.map(president => `${president.first} ${president.last}`)
...
// The first time the callback is called, accumulator and currentValue can be one of two values. If initialValue is provided in the call to reduce(), then accumulator will be equal to initialValue, and currentValue will be equal to the first value in the array. If no initialValue is provided, then accumulator will be equal to the first value in the array, and currentValue will be equal to the second. Here we need an initialValue, otherwise our accumulator (which will be counting for us) will be an item in our array.
...
let yearsLived = presidents.reduce((accumulator, currentValue) => {
    ...
    if(currentValue.passed === undefined){
        ...
        let yearsLived = currentValue.passed - currentValue.year
        sum = accumulator + yearsLived;
    }

    // TERNARY
    // currentValue.passed === undefined ? sum = accumulator + 2019 - currentValue.year : sum = accumulator + currentValue.passed - currentValue.year
